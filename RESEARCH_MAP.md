...
Q1–Q5 的当前权威定义以 `topics/README.md` 为准。它们是长期问题身份，不要求措辞永久冻结，但不得在保留同一 ID 的情况下静默替换成另一个问题。

### Problem Semantic Evolution｜问题语义演化

研究深入后，问题允许发生精炼、分裂、重构与替代。仓库不阻止这种演化，但必须保留问题谱系。

允许使用以下关系描述变化：

- `refines`：对原问题进一步精炼；
- `splits_into`：一个问题分化成多个问题；
- `derived_from`：新问题由旧问题、Evidence 或现实 Trigger 派生；
- `reframes`：问题的观察框架发生实质重构；
- `merges`：多个问题合并；
- `related_to`：存在关联，但不是演化关系；
- `supersedes`：新问题明确替代旧问题。

核心规则：

> **问题可以演化，但 ID 不应偷偷换意思。**

当原问题已经无法覆盖新的研究对象时，优先创建 RP / 子问题并建立关系；若确需重新定义原问题，应在 `EVOLUTION.md` 中显式记录 reframe / supersede，而不是直接覆盖历史身份。
