...
- **当前处理**：当 R-001 进入“aggregation condition”子问题时可直接升级为 **A 类**；在此之前保持候选，不因篇幅或名气提前开启。
...
- **当前处理**：对当前 R-001 属于 **C**，不打断 P-002；当 Active Problem 转向 Enterprise Context / World Model、Agent 自主性或跨主体编排时升级为 **B**，再定向阅读 L3→L4、L4→L5 与证据边界。

## 4. 阅读顺序与收集边界

- **当前驱动对象是 R-001，不是 P-002。**
- 原计划 `P-001 → P-002 → P-003` 只保留为历史记录，不再承担执行顺序。
- P-002 当前先做 B 类定向阅读；如果对 R-001 有直接重大增量再升级 A。
- 如果 R-001 转入“聚合条件”，P-005 比按编号继续 P-003 更有优先级。
- 如果 R-001 转入 Multi-Agent 组织失败，再考虑 P-004。
- P-006 对当前 R-001 只作 C 类登记；不要因为“世界模型”概念前沿就改写当前研究顺序。
