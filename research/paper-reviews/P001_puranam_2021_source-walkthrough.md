...
**状态：P-001 本轮精读完成。**本文件承担 Part A 原文导读与 Part B 概念还原；Part C 七问及 Part D 吸收记录见[研究卡](P001_puranam_2021_hacd.md)。下文原有详细导读保留。

原文依据：[Puranam (2021), Journal of Organization Design, 10:75–80](https://doi.org/10.1007/s41469-021-00095-2)。作者提供概念设计空间，未用新数据集验证一种普遍最优结构。

[原文] 文章的论证顺序是：把人机共同决策视为组织设计问题 → 解释合作的可能收益（Figure 1）→ 展开分工结构（Figure 2）→ 因双方会适应而进入学习信息条件（Table 1）→ 将分工与学习联合考虑。

| 需要区分的内容 | 回答什么 | 不能混成什么 |
|---|---|---|
| 专业化收益 / 聚合收益 | 为什么合作可能有价值？ | Figure 2 的两个分类维度 |
| Figure 2：相同 / 不同决策任务 × 并行 / 串行 | 决策工作怎样安排？ | AI 做基础、人做高级的固定分工 |
| Table 1：独立 / 相互依赖的反馈 × 交流受限 / 可行 | 各方在什么信息条件下学习？ | 仅凭反馈类型即可确定四种配置 |
| 输入筛选、相互调整、错误归因 | 结构怎样影响学习机会与学习结果？ | 与四种学习配置互相排斥的另一套类别 |

[背景补充｜学习用归纳] 可用四个问题恢复两张四格表：**做什么？怎样衔接？收到什么反馈？能否借鉴对方？**

专业化可采用串行或并行；聚合收益不是“异构顺序”的另一种叫法。任务依赖、反馈依赖、交流条件分别描述不同关系。概念的价值在于解释机制，不要求靠背诵四个英文名称证明理解。

标记规则：[原文] 为文章明确陈述；[作者引用的既有研究] 为作者借用的理论或结果；[背景补充] 为教学解释与自拟例子；[我们的推论] 为进一步研究解释。概念分类不能直接证明企业应用效果。
...
[背景补充] 如果两个判断者的错误不是完全同方向，适当聚合后，一部分错误可能互相抵消；实际收益还取决于偏差、误差相关性与聚合规则。原文引用了聚合与 error cancellation 的已有思想，但没有给出企业选型所需的完整数学条件。
...
[作者引用的既有研究] 作者采用 Argote (2013) 的宽定义：
...
[原文] Learning Configuration 描述各主体可用于学习的信息条件。[背景补充] 可以先把它理解成“各方能得到什么反馈，以及有什么机会了解他人经验”。它可以由规则设计形成，也可能是既有工作结构造成的条件。
...
这时两边的学习被绑在一起，作者称为 **coupled learning**。这里用“只评价共同结果”举例；更一般的标准是给一方的反馈会受另一方决策影响，反过来也如此。
...
### ③ Coupled Learning（表中仅有耦合、交流受限的一格）

`Interdependent Feedback + Communication Constrained`

两边只能看到共同结果，却无法充分看见对方如何做。

这里存在 attribution problem：结果很好，到底是谁做对了？结果很差，到底是谁做错了？

**Coupled 本身只表示反馈依赖，不包含“难看清对方”这个条件。**交流可行时同样可以 Coupled，即下一格。
...
这并不自动代表最好，只是信息结构更丰富。作者为了简化分类，将一定程度的经验交流可行归入 Vicarious；实际借鉴还需要主体能够利用这些信息。独立反馈本身不能推出 Vicarious，看到对方过程也不保证能够正确归因。
...
[作者引用的既有研究] 在 Serial / Sequential Architecture 中，上游可能把一部分案例过滤掉，从而限制下游的学习机会；作者在结尾引用 Christensen 与 Knudsen 的研究讨论这一点。
...
[背景补充｜虚构教学情境] 两名木匠共同做桌子，甲做桌腿，乙做桌面。木匠类比见原文；以下检测、交流和销售情节为教学扩展，不是新增企业证据。

- 工作存在依赖：接口需要匹配；这不等于反馈一定耦合。
- 按约定标准分别检测部件，可以向各方提供针对自己贡献的反馈。此处假设检测能区分相应贡献，不宣称现实中总能做到。
- 仅向双方反馈整桌销量，不能据此单独评价桌腿或桌面的贡献。把同一个整体结果分别发给两人，也不会变成独立反馈。
- 如果双方各有贡献反馈、又能观察并利用对方的经验，在 Table 1 中对应 Vicarious Learning；若不能了解对方经验，则对应 Isolated Learning。
- 如果只得到共同结果，同时能从对方经验中学习，则对应 Coupled + Vicarious Learning。
- 甲若从“桌子卖得好”直接推出“自己的桌腿工艺正确”，并强化该做法，存在错误归因与 Superstitious Learning 的风险。这里不能反过来断言其工艺必然差；问题在于证据不足。
- 能交流不保证正确归因；反馈相互依赖也不意味着一定学错。
- 学习配置描述信息条件，Censored Inputs 描述可见样本受筛选，Superstitious Learning 描述错误归因形成或强化错误认识。它们可在同一流程中同时出现。

木匠情境适合解释反馈和归因，但实体部件不适合展示“多个判断通过聚合抵消错误”。理解 Figure 1/2 时仍应回到原文的股票判断例子或明确标记的预测教学例子。

# 17｜本篇结束在哪里？

本轮完成的是原文理解与研究解释。后续“什么企业任务适合哪一种安排”需加入任务条件、收益与失败条件、公开证据和比较验证，不能从四格表直接读出唯一答案。

应用建议、补充证据与待办 U-P001-01 统一放在[研究卡](P001_puranam_2021_hacd.md)，不混入作者原文。完成状态与下一步以 [NOW](../../NOW.md) 为准。
